...
public partial class CharacterWindow: Gtk.Window
{	
	...
	public CharacterWindow (): base (Gtk.WindowType.Toplevel)
	{
		Build ();
	...
	public CharacterWindow (Character c): base (Gtk.WindowType.Toplevel)
	{
		Build ();
		rangedWeaponManifests = new List<IManifest<IManifestItem>>();
		meleeWeaponManifests = new List<IManifest<IManifestItem>>();
	...
	protected void SubmitCharacter_Click (object sender, EventArgs e)
	{
		try {
			...
			new MessageWindow("Character Import Window", "Error: " + ex.Message);
		}
	}
	#endregion
